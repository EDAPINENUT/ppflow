...
from ppflow.modules.common.geometry import apply_rotation_to_vector, quaternion_1ijk_to_rotation_matrix
from ppflow.modules.common.so3 import so3vec_to_rotation, rotation_to_so3vec, random_uniform_so3
...
class EpsilonNet(nn.Module):
    ...
    def forward(self, v_t, p_t, s_t, res_feat, pair_feat, beta, mask_gen_pos, mask_res):
        """
        Args:
            v_t:    (N, L, 3).
            p_t:    (N, L, 3).
        ...
        eps_pos = torch.where(mask_gen_pos[:, :, None].expand_as(eps_pos), eps_pos, torch.zeros_like(eps_pos))
        ...
        v_next = torch.where(mask_gen_pos[:, :, None].expand_as(v_next), v_next, v_t)
...
class FullDPM(nn.Module):
    ...
    def forward(self, v_0, p_0, s_0, res_feat, pair_feat, mask_gen_pos, mask_gen_aa,
                mask_res, denoise_structure, denoise_sequence, t=None):
        ...
        if denoise_structure:
            ...
            v_noisy, _ = self.trans_rot.add_noise(v_0, mask_gen_pos, t)
            # Add noise to positions
            p_noisy, eps_p = self.trans_pos.add_noise(p_0, mask_gen_pos, t)
        else:
            R_0 = so3vec_to_rotation(v_0)
            v_noisy = v_0.clone()
            p_noisy = p_0.clone()
        ...
        if denoise_sequence:
            ...
            _, s_noisy = self.trans_seq.add_noise(s_0, mask_gen_aa, t)
        else:
            s_noisy = s_0.clone()

        beta = self.trans_pos.var_sched.betas[t]
        v_pred, R_pred, eps_p_pred, c_denoised = self.eps_net(
            v_noisy, p_noisy, s_noisy, res_feat, pair_feat, beta, mask_gen_pos, mask_res
        ...
        loss_pos = (loss_pos * mask_gen_pos).sum() / (mask_gen_pos.sum().float() + 1e-8)
        ...
        loss_seq = (kldiv * mask_gen_aa).sum() / (mask_gen_aa.sum().float() + 1e-8)
    ...
    def sample(
        ...
        if sample_structure:
            ...
            v_init = torch.where(mask_gen_pos[:, :, None].expand_as(v), v_rand, v)
            p_init = torch.where(mask_gen_pos[:, :, None].expand_as(p), p_rand, p)
        else:
            v_init, p_init = v, p
        ...
        for t in pbar(range(self.num_steps, 0, -1)):
            ...
            p_next = self.trans_pos.denoise(p_t, eps_p, mask_gen_pos, t_tensor)
            _, s_next = self.trans_seq.denoise(s_t, c_denoised, mask_gen_aa, t_tensor)
    ...
    def optimize(
        ...
        if sample_structure:
            # Add noise to rotation
            v_noisy, _ = self.trans_rot.add_noise(v, mask_gen_pos, t)
            # Add noise to positions
            p_noisy, _ = self.trans_pos.add_noise(p, mask_gen_pos, t)
            v_init = torch.where(mask_gen_pos[:, :, None].expand_as(v), v_noisy, v)
            p_init = torch.where(mask_gen_pos[:, :, None].expand_as(p), p_noisy, p)
        else:
            v_init, p_init = v, p

        if sample_sequence:
            _, s_noisy = self.trans_seq.add_noise(s, mask_gen_aa, t)
            s_init = torch.where(mask_gen_aa, s_noisy, s)
        else:
            s_init = s
