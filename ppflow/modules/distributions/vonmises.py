...
class VonMisesMix(Distribution):
    ...
    def log_prob_samples(self, value):
        log_prob_x = self.mixture_model.component_distribution.log_prob_samples(value)
        mix_logits = self.mixture_model.mixture_distribution.logits.unsqueeze(0)
        return torch.logsumexp(log_prob_x + mix_logits, dim=-1)
...
class VonMises(Distribution):
    ...
    def log_prob_samples(self, value):
        ...
        concentration = self.concentration.unsqueeze(0)
        loc = self.loc.unsqueeze(0)
        log_prob = concentration * torch.cos(value - loc)
